...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # The action's preconditions needs to be in the current level
            # of literal lists in order to be valid for adding
            if node_a.prenodes.issubset(self.s_levels[level]):
                # add the action to the list
                a_nodes.append(node_a)

                # create the links with the literal nodes
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_a)
                    node_a.parents.add(node_s)

        # Add a new level to the a_levels
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            for eff_node in node_a.effnodes:
                # add the eff_node to the set
                s_nodes.add(eff_node)
    
                # create links with the action level
                node_a.children.add(eff_node)
                eff_node.parents.add(node_a)
    
        # Add a new level to the s_levels
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Check to see if node_a1's action has negated effects of actions in node_a2
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # Check to see if node_a2's action has negated effects of actions in node_a1
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Check to see if node_a1's action has negated preconditions of actions
        # in node_a2
        for effect in node_a1.action.effect_add:
        ...
        # Check to see if node_a2's action has negated preconditions of actions
        # in node_a1
        for effect in node_a2.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Check the parents of both nodes for mutex. Parents are the preconditions
        # literal of the current action
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # Check the symbol to see if they are identical then
        # check the is_pos to see if they are mutally exclusive
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # Check in the parent node for mutex between the two nodes. If there is
        # even 1 node that is not mutex, we return false
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for node_s in self.s_levels[level]:
                    # Since the goal is in the expr form, we transfer the node to the
                    # the same form for comparison
                    if node_s.is_pos:
                        node_s_literal = expr(node_s.symbol)
                    else:
                        node_s_literal = expr("~{}".format(node_s.symbol))
                    
                    # Do the compare
                    if goal == node_s_literal:
                        level_sum += level
                        is_found = True
                        break
                # If the goal is found, we also break the level for loop
                if is_found:
                    break
